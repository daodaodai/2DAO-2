...
elseif itemsCount == 1 then  -- 1 item selected
	...
	for i = 1, itemsCount do
		...
		if i == 1 then
			...
			-- Get the 1st item's name
			take = reaper.GetActiveTake(item)
			if take then
				firstItemName = reaper.GetTakeName(take)
			end
		else
...
bOK = true
--     算了，不问了，直接上改名步骤吧   --
--bOK, userInput = reaper.GetUserInputs("Rename to the 1st item's name?", 1, "Rename? 0 - No | 1 - Yes", userInput)
if bOK then
	bRename = tonumber(userInput:match("([^,]+)"))
	if bRename == 1 then
		--item = reaper.GetSelectedMediaItem(thisProject, 0)
		--take = reaper.GetActiveTake(item)
		--reaper.GetSetMediaItemTakeInfo_String(take, "P_NAME", firstItemName:gsub(".glued",""), 1)
		--  call:  Xenakios/SWS: Rename take source files... (no undo)
		cmdID = reaper.NamedCommandLookup("_XENAKIOS_RENMTAKEANDSOURCE")
